<template>
  <div class="upload-box flex a-center j-center">
    <div class="show-box">
      <img class="img" v-if="file1 == ''" :src="require(`~/static/small/${src}`)" alt="default" />
      <img class="img" v-else :src="file1" alt="upload" />
    </div>

    <div class="input-box flex flex-col items-center justify-center">
      <div :class="{ activeInfo: file1 != '' }" class="file-info">
        <p>{{ fileName }}</p>
        <p>{{ fileSize }} MB</p>
      </div>
      <label :class="{ invalid: imgInvalid }" @click="(file = ''), (fileName = ''), (fileSize = '')" for="image1"
        class="label flex items-center justify-center">
        <span class="svg-form">
          <UploadSvg />
        </span>
      </label>
      <input multiple id="image1" type="file" accept="image/jpeg, image/jpg" ref="file" name="file1"
